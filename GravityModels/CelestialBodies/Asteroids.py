...
class Bennu:
    def __init__(self):
        ...
        self.sh_10 = pooch.retrieve(
            ...
            path=os.path.dirname(GravityModels.__file__) + "/Files/SH_coefficients/Bennu/",
            processor=self.format_sh
        )
    ...
    # Spherical Harmonics
    def format_sh(self, fname, action, pooch_inst):
        ...
        for i in range(len(names)):
            name_entries = [entry.replace(" ","").replace("'", "").replace("BENNU_", "").replace("...", "") for entry in names[i].split("' '")]
        ...
        for k in range(len(name_list)):
            ...
            if coef == "C":
                data_matrix[(i-2)*max_deg + j] = np.array([i, j, value, data_matrix[(i-2)*max_deg + j, 3], 0.0, 0.0])
            else:
                data_matrix[(i-2)*max_deg + j] = np.array([i, j, data_matrix[(i-2)*max_deg + j, 2], value, 0.0, 0.0])
...
class Eros:
    def __init__(self):
        ...
        self.sh_file = pooch.retrieve(
            ...
            path=os.path.dirname(GravityModels.__file__) + "/Files/SH_coefficients/Eros/",
            processor=self.format_sh
        )


    def reindex_faces(self, fname, action, pooch_inst):
        "add 1 to the face indices in the obj file to work with trimesh"
        new_name = fname.split("_raw")[0] + ".obj"
        if os.path.exists(new_name):
            return new_name
    ...
    def format_sh(self, fname, action, pooch_inst):
        ...
        with open(processed_name, 'w') as f:
            ...
            write_data = [",".join(x) for x in entries]
